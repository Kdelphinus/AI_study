    ...
    def polyreg_helper(degree):
        ...
        if regularized:
            ...
            plt.plot(
                ...
                label="predicted, w/o regularization",
            )
            plt.plot(
                x_plot,
                ridge.predict(x_plot),
                label="predicted, with regularization",
            )
            plt.title(
                f"Poly degree = {degree:2}, MAE_no_reg = {mae_linear:.3f}, MAE_reg = {mae_ridge:.3f}",
                fontsize=16,
            )
            if verbose:
            ...
            plt.title(
                f"Polynomial degree = {degree:2}, MAE = {mae_linear:.3f}",
                fontsize=16,
            )
...
class Arrow2D(FancyArrowPatch):
    ...
    def __init__(self, xs, ys, *args, **kwargs):
        ...
        FancyArrowPatch.__init__(
            self, (xs[0], ys[0]), (xs[1], ys[1]), *args, **kwargs
        )
...
class PlaneColorPicker:
    ...
    def __init__(self, start=0):
        if not PlaneColorPicker.instance:
            PlaneColorPicker.instance = PlaneColorPicker.__PlaneColorPicker(
                start
            )
